...
import { clampPageNumber, clampPaginationLimit, validateRatings, isValidUUID } from "@/lib/utils/validation";
...
export async function GET(request: Request) {
  ...
  // ✅ BUG-03 FIX: Validate hostel_id format if provided
  if (hostelId && !isValidUUID(hostelId)) {
    return NextResponse.json({ data: null, error: "Invalid hostel ID format" }, { status: 400 });
  }
...
export async function POST(request: Request) {
  ...
  // ✅ BUG-03 FIX: Strict UUID validation on hostel_id
  if (!isValidUUID(body.hostel_id)) {
    return NextResponse.json({ error: "Invalid hostel ID format" }, { status: 400 });
  }

  if (typeof body.title !== "string" || typeof body.content !== "string" || !body.rating || typeof body.rating !== "object") {
  ...
  if (!validatedRatings) return NextResponse.json({ error: "All ratings must be integers 1-5" }, { status: 400 });

  const validDurations = ["Less than 3 months", "3-6 months", "6 months - 1 year", "1-2 years", "2+ years"];
  const stayDuration = body.stay_duration && typeof body.stay_duration === "string" && validDurations.includes(body.stay_duration)
    ? body.stay_duration : null;

  const { data: review, error: re } = await supabase.from("reviews")
    .insert({ hostel_id: body.hostel_id, user_id: user.id, title, content, stay_duration: stayDuration })
    .select().single();

  if (re) {
    if (re.code === "23505") return NextResponse.json({ error: "You already reviewed this hostel" }, { status: 409 });
  ...
  if (ratErr) {
    ...
    return NextResponse.json({ error: "Failed to save ratings" }, { status: 400 });
  }
